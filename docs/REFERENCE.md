...
| `ai-sdlc-skills-handoff` | Context usage nears ~40–50%, or the user asks to hand off / resume | `HANDOFF.md` state snapshot (goal, state, decisions, remaining work, traps, resume command, files to load); `check` blocks on unfilled markers, leftover template text, empty decision rows and unexecutable defaults |
