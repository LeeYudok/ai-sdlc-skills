...
이슈 우선 규칙이 있는 저장소에서는 `ai-sdlc-skills-issue`가 이슈 번호·기획서를 forge 이슈와
`<type>/issue-N-<slug>` 브랜치로 바꾸고 run slug를 이슈에 고정한다. 구현 단계는 테스트 작성
패스를 제품 코드 패스와 분리해(가능하면 별도 서브에이전트) 테스트가 구현이 아니라 수용 기준을
담게 한다.
...
사용자는 **무엇을(What) · 왜(Why)** 만 주면 된다. 참고자료·예시 화면·우선순위는 선택이며,
없다고 해서 작업이 멈추지 않는다. 나머지(요구사항 정리, 완료 기준, 영향 범위, 검증 방법)는
파이프라인이 저장소 증거에서 만들어 문서로 남기고, 해석이 갈리는 지점만 되묻는다.
