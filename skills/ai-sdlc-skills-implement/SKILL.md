...
End `implementation-report.md` with a machine-readable gate verdict line — `Verdict: PASS` when the change matches the approved scope and focused checks pass, otherwise `Verdict: BLOCKED` naming the specification or impact error found. `pipeline_state.py` reads this line to allow or block the `implemented` transition; the last such line in the file wins.
